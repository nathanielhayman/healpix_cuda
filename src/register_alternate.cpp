...
static int get_ring_and_pix_z (long nside_, double z, double phi, int* ring)
{
  double za = fabs(z);
  ...
  if (za<=twothird) /* Equatorial region */
    ...
    /* ring number counted frosm z=2/3 */
...
void get_ring_and_pix(long nside, double theta, double phi, int* ring, long *ipix)
{
  *ipix=get_ring_and_pix_z (nside,cos(theta),phi,ring);
}
    ...
    int height, int n_side, pointing* fov, T_Healpix_Base<int>* hp) 
{
    ...
    std::vector<int> a_ring(4);
    std::vector<long> a_pix(4);
    ...
    for (int i = 0; i < 4; ++i) {
        int ring = 0;
        long pix = 0;
        get_ring_and_pix(n_side, vecs[i].theta, vecs[i].phi, &ring, &pix);

        if (i == 1 && a_ring[0] != ring)
            printf("pixel boundaries on different rings!!\n");
